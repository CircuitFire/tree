...
impl<T> Tree<T> {
    ...
    fn all_descendants_helper(&self, id: usize, ids: &mut Vec<usize>){
        let mut child = self.nodes[id].first_child;

        while let Some(child_id) = child {
            ids.push(child_id);
            self.all_descendants_helper(child_id, ids);
    ...
    pub fn all_descendants_of(&self, id: usize) -> Result<Vec<usize>, TreeErr> {
        self.valid_node(id)?;

        let mut ids = Vec::with_capacity(self.len());
        self.all_descendants_helper(id, &mut ids);

        Ok(ids)
    }

    /// Returns a list starting with the id provided followed by all of its descendants.
    pub fn all_nodes_from(&self, id: usize) -> Result<Vec<usize>, TreeErr> {
        self.valid_node(id)?;
    ...
    fn all_nodes_counted_helper(&self, id: usize, ids: &mut Vec<NodeChildren>){
        let index = ids.len();
        ...
        while let Some(child_id) = child {
            ...
            self.all_nodes_counted_helper(child_id, ids);
            child = self.nodes[child_id].next_sib;
        }
    }
    ...
    pub fn all_nodes_from_counted(&self, id: usize) -> Result<Vec<NodeChildren>, TreeErr> {
        self.valid_node(id)?;

        let mut ids = Vec::new();
        self.all_nodes_counted_helper(id, &mut ids);
    ...
    pub fn remove(&mut self, id: usize) -> Result<(), TreeErr> {
        ...
        for child in self.all_descendants_of(id).unwrap() {
    ...
    fn valid_move(&self, moving: usize, new_place: usize) -> Result<(), TreeErr> {
        ...
        for id in self.all_nodes_from(moving).unwrap() {
...
impl<'a, T: IntoBytes<'a>> Iterator for TreeIter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item>{
        if let Some(value) = self.node_iter.next(){
            Some(value)
        }
        else {
            if let Some(node) = self.nodes_iter.next(){
                self.data = Some(self.tree.data_at(node.id).unwrap());
                let child_count = node.child_count as u32;
                self.node_iter = Box::new(self.data.unwrap().into_bytes().chain(child_count.into_bytes_static()));
                self.node_iter.next()
            }
            else { None }
        }
